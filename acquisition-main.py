...
from acquisition.goods import dictGoods, dictGoodsZh, dictPrice, dictEffic
...
class calculate:
    ...
    def sellOut(self, player, itemToSell):
        ...
        itemID = dictGoods.index(itemToSell)
        itemToSellName = dictGoodsZh[itemID].decode('utf-8')
        ...
        priceInit = dictPrice[itemID]
        goodsEffic = dictEffic[itemID]
...
class GUIselect:
    def __init__(self, player, itemToSell):
        ...
        self.itemID = dictGoods.index(self.itemToSell)
        self.itemToSellName = dictGoodsZh[self.itemID].decode('utf-8')
...
def newCycle(sender, label, args):
    ...
    if player.hasPermission("acq.admin"):
        ...
        for sectionName in historyDetailDict:
            ...
            elif str(sectionName)[len(playerName)+1:] in dictGoods:
