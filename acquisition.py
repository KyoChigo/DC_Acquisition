...
class calculate:
    def __init__(self):
        ...
        self.dictPrice = [20.00, 6.00, 1.50, 0.85, 1.00, 1.00, 0.08]
...
def test(sender, label, args):
    player = sender.getPlayer()  # 获取玩家对象
    ItemInHand = player.getInventory().getItemInMainHand()  # 手中物品

    if ItemInHand.getType().toString() in calculate().dictGoods:
        player.sendMessage(ChatColor.translateAlternateColorCodes('&', u"&e[DC收购]&a 正在收购 " + ItemInHand.getType().toString() + u"！"))
        ItemInHandNumber = 0  # 手中物品的总数
        # 遍历玩家的背包
        for item in player.getInventory().getContents():
            if item is not None and item.getType() == ItemInHand.getType():
                ItemInHandNumber += item.getAmount()
        ...
        def clickHandler(slot, stateSnapshot):
            if slot == AnvilGUI.Slot.OUTPUT: # GUI输入
                ...
                if text is not None:
                    ...
                    if int(text) > ItemInHandNumber: # 限制出售物品数量的上下限
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', u"&e[DC收购]&c 您只有 " + str(ItemInHandNumber) + u" 个 "
                                                                                  + ItemInHand.getType().toString() + u" ！"))
                        return [AnvilGUI.ResponseAction.replaceInputText(u"您没有那么多物品！")]
                    elif int(text) <= 0:
        ...
        def closeHandler(stateSnapshot):
            if itemNumHolder[0] >= 1 and itemNumHolder[0] <= ItemInHandNumber:
                ...
                if historyDetailSection is not None:   # 检查玩家是否有收购记录
                    ...
                    testHistory = 0
                itemID = calculate().dictGoods.index(ItemInHand.getType().toString())
                ...
                player.getInventory().removeItem(org.bukkit.inventory.ItemStack(ItemInHand.getType(), countSold)) # 删除出售物品
                residue -= testprice
                if countSold == 0:
                    ...
                    player.sendMessage(ChatColor.translateAlternateColorCodes('&', u"&e[DC收购]&a 售出 " + str(countSold) + u" 个 " + ItemInHand.getType().toString() 
                                                                            + u"，获得 " + str(testprice) + u" DC币！平均单价为 " + str(testUnitPrice) + u" DC币。"))
                    player.sendMessage(ChatColor.translateAlternateColorCodes('&', u"&e[DC收购]&a 本周期剩余收购额度 " + str(residue) + u" DC币。"))

                tempDict["RESIDUE"] = residue
                try:
                    tempDict[ItemInHand.getType().toString()][0] += countSold
                except:
        ...
        player.sendMessage(ChatColor.translateAlternateColorCodes('&', u"&e[DC收购]&c 手中物品不在收购范围内！"))
...
ps.command.registerCommand(test, "functest")
